...
#include "src/cv.cpp"  // Include the original CV module (no changes)
...
CVBenchmarkResult measureCVTrainingTime(const DataFrame& data, int depth, const std::string& dataset_name, 
                                       int warmup_runs = 1, int measurement_runs = 1) {
    /**
     * Manually perform CV to measure only training time, not prediction/evaluation
     */
    ...
    // Measurement runs - time only training
    std::vector<double> total_times;
    std::vector<double> fold_scores;
    
    for (int m = 0; m < measurement_runs; m++) {
        double total_training_time = 0.0;
        std::vector<double> current_fold_scores;
        ...
        for (int fold = 0; fold < 4; fold++) {
            ...
            total_training_time += duration.count() / 1000.0;
            ...
            current_fold_scores.push_back(fold_accuracy);
        }
        
        total_times.push_back(total_training_time);
...
std::vector<CVBenchmarkResult> testDatasetCV(const std::string& dataset_path, const std::string& dataset_name) {
    ...
    for (int depth : depths) {
        try {
            std::cout << "Testing SERIAL CV with depth=" << depth << " (training time only)..." << std::flush;
            
            CVBenchmarkResult result = measureCVTrainingTime(df, depth, dataset_name, warmup_runs, measurement_runs);
...
int main() {
    std::cout << "=== SERIAL Cross-Validation Benchmark (Training Time Only) ===" << std::endl;
    std::cout << "Using manual CV implementation to measure only training time" << std::endl;
    ...
    std::cout << "IMPORTANT: Times represent training only, comparable to tree benchmark times" << std::endl;
    std::cout << "Expected CV training time ranges:" << std::endl;
    std::cout << "  Depth 1-5: 4-200ms (sum of 4 training times)" << std::endl;
    std::cout << "  Depth 6-12: 40-2000ms (sum of 4 training times)" << std::endl;
    std::cout << "  Depth 15-20: 200-8000ms (sum of 4 training times)" << std::endl;
